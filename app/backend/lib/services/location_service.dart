...
class LocationService {
  ...
  // Vehicle detection — 30 km/h threshold; 3 consecutive readings trigger warning
  static const double _vehicleSpeedThresholdMps = 8.33; // 30 km/h
  static const int _vehicleConsecutiveThreshold = 3;
  int _vehicleHighSpeedCount = 0;

  /// Called with the current speed (m/s) whenever a sustained vehicle-like
  /// speed is detected.  Reset to null when speed drops back to normal.
  Function(double speedMps)? onVehicleSpeedDetected;
  Function()? onVehicleSpeedNormal;

  // GPS calibration (allow jumps for first few updates)
  int _gpsPositionCount = 0;
  static const int _gpsCalibrationUpdates =
      3; // Quick GPS stabilization (was 10 — too slow)
  ...
  void startTracking(
    ...
    Function(LatLng location, double accuracyMeters)?
    onLocationUpdateWithAccuracy,
  }) {
    ...
    _positionStreamSubscription =
        Geolocator.getPositionStream(
          ...
          if (!_isCalibrated) {
            ...
            // First GPS fix is often from cell tower / stale cache — skip it
            print(
              '✅ Sensor calibration done — first GPS fix received, stabilizing...',
            );
            return;
          }
          ...
          // GPS calibration: let positions flow through (UI responsive)
          // Outliers are cleaned by _sanitizePath() before territory creation
          if (isGpsCalibrating) {
            ...
            onLocationUpdateWithAccuracy?.call(location, position.accuracy);
            print(
              '🎯 GPS calibrating... (${_gpsPositionCount}/$_gpsCalibrationUpdates) pos: ${location.latitude.toStringAsFixed(5)}, ${location.longitude.toStringAsFixed(5)}',
            );

            if (_gpsPositionCount == _gpsCalibrationUpdates) {
              print('✅ GPS calibration complete — full tracking active');
            }
            return;
          }
          ...
          // GPS distanceFilter already ensures 3m minimum displacement.
          // Accept if sensor says moving OR distance ≥ 3m (don't let noisy
          // sensor oscillation block real walking positions).
          if (isMovingBySensor || distance >= 3) {
            // ── Vehicle speed detection ──────────────────────────────────────
            final timeDiff2 =
                DateTime.now().difference(_lastPositionTime!).inMilliseconds /
                1000.0;
            if (timeDiff2 > 0) {
              final currentSpeedMps = distance / timeDiff2;
              if (currentSpeedMps >= _vehicleSpeedThresholdMps) {
                _vehicleHighSpeedCount++;
                if (_vehicleHighSpeedCount >= _vehicleConsecutiveThreshold) {
                  onVehicleSpeedDetected?.call(currentSpeedMps);
                }
              } else {
                if (_vehicleHighSpeedCount >= _vehicleConsecutiveThreshold) {
                  onVehicleSpeedNormal?.call();
                }
                _vehicleHighSpeedCount = 0;
              }
            }
            // ────────────────────────────────────────────────────────────────
            ...
            onLocationUpdateWithAccuracy?.call(location, position.accuracy);
            print(
              '📍 Track #${_currentPath.length}: (${location.latitude.toStringAsFixed(5)}, ${location.longitude.toStringAsFixed(5)}) +${distance.toStringAsFixed(1)}m',
            );
          } else {
  ...
  /// Call this to reset the vehicle counter when the user confirms they are
  /// still on foot ("I'm walking" button in the warning dialog).
